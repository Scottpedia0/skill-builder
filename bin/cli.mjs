    ...
    case "implement": {
      ...
      if (result.validation) {
        ...
        if (result.validation.warnings.length) {
          console.error(`\n⚠️  Warnings:`);
          for (const w of result.validation.warnings) console.error(`   ${w}`);
        }
      ...
      if (dryRun) {
        console.log(`\n[DRY RUN] Would install to: ${result.path}`);
        if (result.llmGenerated) console.log(`  Generated by: AI model`);
        console.log(`\n${result.content}`);
      } else if (result.error) {
        console.error(`\n${result.error}`);
        console.error("Fix the issues above or use --dry-run to preview the content.");
        process.exit(1);
      } else {
