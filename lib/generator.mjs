...
function buildSkillPrompt(suggestion, config) {
  ...
  return `You are an expert at building automation skills for AI coding agents (Claude Code, Codex, Cursor, Gemini CLI).

A skill is a markdown file with YAML frontmatter that an AI agent can execute as a slash command. Skills contain REAL, RUNNABLE commands — bash scripts, curl calls, SQL queries, git commands. NOT outlines or TODOs.
...
Generate a complete, working SKILL.md file that automates this pattern. The skill must:

1. Have proper YAML frontmatter with \`name\` and \`description\` fields
2. Contain ONLY runnable commands in bash code blocks — no placeholders like <your-thing-here>
3. Handle errors gracefully (check if commands exist, handle missing files)
4. Be useful on the FIRST run without additional setup
5. Be concise — under 50 lines of actual commands
...
---
name: skill-name
description: "One line describing when to trigger this skill"
---
...
A good skill for "git checkout main && git pull && git checkout -b feature/" would be:

\`\`\`markdown
---
name: new-branch
description: "Create a new feature branch from a fresh main"
---
...
echo "Created and switched to feature/$branch_name"
\\\`\\\`\\\`
\`\`\`

Now generate the skill for the detected pattern. Output ONLY the markdown content, no explanation before or after.`;
}
